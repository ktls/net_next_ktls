...
static inline bool tls_stream_memory_free(const struct sock *sk)
{
	...
	return tsk->unsent < TLS_MAX_PAYLOAD_SIZE;
}
...
static inline ssize_t tls_read_size(struct tls_sock *tsk, struct sk_buff *skb)
{
	...
	datagram_len = data_len + TLS_OVERHEAD;
	...
	if (tsk->control) {
		printk("Size of non-datagram message is %li\n", datagram_len);
	}
	return datagram_len;
			  ...
			  unsigned int optlen)
{
	...
	if (!TLS_SETSOCKOPT_READY(tsk)) {
			//printk("tls setsockopt not ready, SETUP\n");
			tsk->cipher_type = TLS_CIPHER_AES_GCM_128;
			tsk->cipher_crypto = "rfc5288(gcm(aes))";
			tsk->version[0] = TLS_1_2_MAJOR;
			tsk->version[1] = TLS_1_2_MINOR;
...
//							goto bind_end;
	...
	//printk("after Sock setup in setsockopt...\n");
				    ...
				    size_t plaintext_len)
{
	...
	/* we can use IV for nonce explicit according to spec */
		buf[3] = pkt_len >> 8;
		buf[4] = pkt_len & 0xFF;
		memcpy(buf + TLS_NONCE_OFFSET, tsk->iv_send, TLS_IV_SIZE);
}
...
static int tls_pre_encrypt(struct tls_sock *tsk, size_t data_len)
{
	 ...
	 */
	sg_set_page(sg, tsk->pages_send, PAGE_SIZE - TLS_PREPEND_SIZE,
		    TLS_PREPEND_SIZE);
	for (i = 1; i < npages; i++)
...
static void tls_kernel_sendpage(struct tls_sock *tsk)
{
	...
	ret = kernel_sendpage(
		...
		tsk->send_len + TLS_OVERHEAD - tsk->send_offset,
		MSG_DONTWAIT);

	if (ret > 0) {
		tsk->send_offset += ret;
		if (tsk->send_offset >= tsk->send_len + TLS_OVERHEAD) {
...
static int tls_sendmsg(struct socket *sock, struct msghdr *msg, size_t size)
{
	...
	if (!TLS_SEND_READY(tsk)) {
			//printk("Send not ready\n");
		ret = tsk->socket->ops->sendmsg(tsk->socket, msg, size);
		release_sock(sock->sk);
		return ret;
		ret = -EBADMSG;
		goto send_end;
	...
	if (!tsk->attached) {
		ret = sock_sendmsg(tsk->socket, msg);
		//printk("sock_sendmsg passthrough ret %i\n", ret);
		goto send_end;
	}
			...
			struct sk_buff* skb)
{
	...
	aead_request_set_crypt(msg->aead_req, sgin, sgout,
			       ...
			       (u8 *)header_recv + TLS_NONCE_OFFSET);
...
static struct sk_buff_head* getqueue(struct tls_sock *tsk) {
	struct sk_buff_head* queue;
	if (tsk->attached) {
		queue = &tsk->sk.sk_receive_queue;
	} else {
		queue = &tsk->sk.sk_error_queue;
		//printk("Returning error queue head %p\n", queue);
	}
				     ...
				     long timeo, int *err)
{
	...
	while (!(skb = skb_peek(getqueue(tsk)))) {
		       ...
		       int flags)
{
	...
	if (!TLS_RECV_READY(tsk)) {
			//printk("recv not ready\n");
		err= tsk->socket->ops->recvmsg(tsk->socket, msg, len, flags);
		release_sock(sock->sk);
		return err;
		err = -EBADMSG;
		goto recv_end;
	...
	if (!tsk->attached) {
			//printk("tls_recvmsg without attach\n");
	}
	if (NULL == skb_peek(&tsk->sk.sk_receive_queue) &&
		NULL != skb_peek(&tsk->sk.sk_error_queue) &&
		tsk->attached) {
		err = -EBADMSG;
		goto recv_end;
	}
...
static int tls_bind(struct socket *sock, struct sockaddr *uaddr, int addr_len)
{
	...
	ret = sock_create(uaddr->sa_family, SOCK_STREAM, IPPROTO_TCP, &tsk->socket);
	if (ret < 0)
			return ret;
	return kernel_bind(tsk->socket, uaddr, addr_len);
}
...
static int tls_listen(struct socket *sock, int backlog) {
	struct tls_sock *tsk;

	tsk = tls_sk(sock->sk);

	return kernel_listen(tsk->socket, backlog);
}
...
static int tls_accept(struct socket *sock, struct socket *newsock, int flags) {
	struct tls_sock *tsk;
	struct tls_sock *newtsk;

	tsk = tls_sk(sock->sk);
	newtsk = tls_sk(newsock->sk);

	int ret;

	tls_create(sock_net(sock->sk), newsock, 0, 0);
	newtsk = tls_sk(newsock->sk);
	//printk("Accepting to sock %p %p\n", newtsk->socket, newtsk);
	ret = kernel_accept(tsk->socket, &newtsk->socket, flags);
	//printk("Accept ret is %i\n", ret);
	if (ret < 0) {
			newtsk->socket = NULL;
			return ret;
	}
	//printk("Accepted, recvmsg is %p socket is %p\n", newtsk->socket, newtsk->socket->sk->sk_prot->recvmsg);
	return ret;
}

static int tls_getname(struct socket *sock, struct sockaddr *uaddr,
				int *uaddr_len,int peer) {
	struct tls_sock *tsk;

	tsk = tls_sk(sock->sk);

	return inet_getname(tsk->socket, uaddr, uaddr_len, peer);
}

static int tls_connect(struct socket *sock, struct sockaddr *addr,
				int addrlen,int flags) {
	struct tls_sock *tsk;
	int ret;

//	printk("tls_connect %p\n", sock);

	tsk = tls_sk(sock->sk);

	// TODO: put BOTH sock_creates somewhere else
	ret = sock_create(addr->sa_family, SOCK_STREAM, IPPROTO_TCP, &tsk->socket);
	if (ret < 0)
			return ret;

	ret = kernel_connect(tsk->socket, addr, addrlen, flags);
	// TODO errorhandling
	struct file *newfile;
	...
	newfile = sock_alloc_file(tsk->socket, flags, NULL);
	if (IS_ERR(newfile))
			return -1; // TODO
